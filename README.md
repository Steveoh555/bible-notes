...
1. **Claude가 자료 페이지를 만듭니다** → `studies/제목.html`
2. **`python3 build.py`** → 홈·사이트맵·RSS가 다시 만들어지고, 모든 페이지에 공통 헤더/푸터와 검색엔진용 태그가 자동으로 채워집니다
3. **`git add -A && git commit -m "..." && git push`** → 몇 분 뒤 사이트에 반영

1~3을 Claude가 한 번에 처리합니다. 직접 하실 일은 없습니다.
